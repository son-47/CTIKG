...
import logging
import re
from ctinexus.mitre_mapper import MitreMapper
...
class PostProcessor:
    def __init__(self, config: DictConfig):
        ...
        # --- KHỞI TẠO MITRE MAPPER ---
        print("🛠️ Initializing MitreMapper in PostProcessor...")
        # Đảm bảo bạn đã có thư mục mitre-ttp-mapping-main ở root
        self.mitre_mapper = MitreMapper(config, dataset_dir="mitre-ttp-mapping-main")
    ...
    def call(self, result: dict) -> dict:
        ...
        # ---------------------------------------------------------
        # PHẦN 2: LOGIC MỚI - ÁNH XẠ MITRE ATT&CK
        # ---------------------------------------------------------
        print("🔍 Running MITRE ATT&CK Mapping...")
        ...
        context_text = result.get("source_text", "") 

        for triple in self.js["EA"]["aligned_triplets"]:
            # Dữ liệu trong triple của bạn có dạng:
            # {'subject': {node info}, 'relation': 'action name', 'object': {node info}}
            
            subj_text = triple.get("subject", {}).get("entity_text", "")
            relation_text = triple.get("relation", "") # Quan hệ thường là string
            obj_text = triple.get("object", {}).get("entity_text", "")
